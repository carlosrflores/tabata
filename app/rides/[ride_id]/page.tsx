...
// Open Graph / Twitter title + description for the link preview that
// iMessage, Slack, etc. show when this URL is pasted. The og:image itself
// is generated by ./opengraph-image.tsx and auto-wired by Next.js.
export async function generateMetadata({ params }: Props): Promise<Metadata> {
  const db = getSupabaseAdmin();
  const { data: ride } = await db
    .from('rides')
    .select('title, instructor_name, duration_seconds')
    .eq('id', params.ride_id)
    .single();

  if (!ride) {
    return { title: 'Tabata Tuesday' };
  }

  const minutes = ride.duration_seconds
    ? Math.round(ride.duration_seconds / 60)
    : null;
  const title = ride.title
    ? `${ride.title} · Tabata Tuesday`
    : 'Tabata Tuesday';
  const description = [
    minutes ? `${minutes} min ride` : null,
    ride.instructor_name ? `with ${ride.instructor_name}` : null,
    'Head-to-head leaderboard for our group.',
  ]
    .filter(Boolean)
    .join(' — ');
